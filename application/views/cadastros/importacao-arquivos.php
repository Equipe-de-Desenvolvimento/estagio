                    ...
                    <tr>                       
                        ...
                        if ($arquivo_pasta != false) {
                            foreach ($arquivo_pasta as $value) {
                                $i++;
                                ?> 
                                <td width="10px"><img  width="50px" height="50px" onclick="javascript:window.open('<?= base_url() . "upload/arquivoestagio/" . $paciente_id . "/" . $item->documentacao_profissional_id . "/" . $value ?>', '_blank', 'toolbar=no,Location=no,menubar=no,width=1200,height=600');" src="<?= base_url() . "img/paste_on.gif" ?>"><br><? echo substr($value, 0, 10) ?><br>
                                    <?php if($perfil_id != 18 && $perfil_id != 20){?>
                                    <a href="<?= base_url() ?>cadastros/pacientes/excluirdocumentacao/<?= @$paciente_id ?>/<?= $item->documentacao_profissional_id; ?>/<?= $value ?>">Excluir</a>
                                    <?php }?>
                                </td>
                                <?
                                if ($i == 8) {
